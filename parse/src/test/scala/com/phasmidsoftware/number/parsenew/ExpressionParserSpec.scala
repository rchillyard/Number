...
class ExpressionParserSpec extends AnyFlatSpec with should.Matchers {
  ...
  it should "parse" in {
    val number = math"1"
    number shouldBe Some(One)
  }

  // TODO Figure out why this (and following cases) doesn't work
  ignore should "apply number expressions" in {
    val number = math"1"
    number shouldBe Some(One)
    val minus1 = math"-1"
    minus1 shouldBe Some(UniFunction(One, Negate))
  ...
  ignore should "apply functions" in {
    ...
    math"\\sin(\\pi * -1)" shouldBe Some(BiFunction(UniFunction(ConstPi, Sine), UniFunction(One, Negate), Product))
