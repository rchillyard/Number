...
import com.phasmidsoftware.number.core.{Factor, Number, Scalar}
...
class NumberParser extends RationalParser {
  ...
  private def number: Parser[Number] = opt(rationalNumber) ~ opt(factor) ^^ { case ro ~ fo => optionalNumber(ro, fo).getOrElse(Number()) }
