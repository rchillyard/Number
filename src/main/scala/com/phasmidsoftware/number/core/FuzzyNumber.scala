...
object FuzzyNumber {
  ...
  private def power(number: FuzzyNumber, p: Number): Number = composeDyadic(number.scale(Scalar), p, p.factor, DyadicOperationPower, independent = false, getPowerCoefficients(number, p))
