...
//
///**
//  * A representation of a mathematical series which is a (possibly) infinite sum of constant terms.
...
//  */
//trait NumberSeries extends IndexedSeq[Number] with Numerical with Approximatable {
//  /**
...
//    * @return a String
//    */
//  def render: String = s"NumberSeries $this"
////    approximation.map(_.render).getOrElse(s"NumberSeries $this cannot be rendered")
//}
//
///**
...
//  * CONSIDER IndexedSeq is not necessary here (and is inappropriate for lazy lists)
//  */
//trait PowerSeries extends NumberSeries with (Number => NumberSeries) {
////  def apply(x: Number): NumberSeries = {
////
////  }
//}
//
...
//  */
//abstract class InfiniteTaylorSeries(point: Number, function: SeriesFunction, derivative: SeriesFunction => SeriesFunction) extends LazyPowerSeries {
//
//  def functions: LazyList[Number=>Number] = LazyList.iterate(function)(f => derivative(f))
//  def coefficients: LazyList[Number] = functions.map(f => f(point))
//  // CONSIDER we could use scanLeft and just multiply by the index (wouldn't need factorial, then)
//  def terms: LazyList[Number] = coefficients.zipWithIndex.map{case (c, i) => c doMultiple Rational(Factorial(i)).invert}
//
//  /**
//    * Evaluates the power series at the specified value of x.
//    *
...
//    */
//  def evaluate(x: Double): Try[Number] = {
//    val qqq = terms.foldLeft((Number.zero, x)){
//      case ((a, z), term) =>
//        (a doAdd (term doMultiply(Number(x)))) -> z * x
//    }
//    Success(qqq._1)
//  }
//
...
//    * @return the term of the power series at the given index as a Number
//    */
//  def getTerm(n: Int): Number = terms(n)
//
//  /**
...
//}
//
//case class TaylorSeries(point: Number, function: SeriesFunction, derivative: SeriesFunction => SeriesFunction) extends InfiniteTaylorSeries(point, function, derivative) {
//
//}
//
//object TaylorSeries {
