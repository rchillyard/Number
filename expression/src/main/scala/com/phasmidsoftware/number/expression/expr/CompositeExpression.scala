...
trait CompositeExpression extends Expression {
  ...
  /**
    * Provides a lazy string representation of the `CompositeExpression`,
    * delegating the rendering logic to the `renderAsExpression` method.
    *
    * This value is computed only once, upon its first access, and caches the result
    * for subsequent accesses. It is intended to present the `CompositeExpression`
    * in a structured or human-readable mathematical format by reusing the string
    * output generated by `renderAsExpression`.
    *
    * @return the string representation of this `CompositeExpression`.
    */
  lazy val render: String = simplify match {
    case exp: CompositeExpression => exp.renderAsExpression
    case exp => exp.render
  }
