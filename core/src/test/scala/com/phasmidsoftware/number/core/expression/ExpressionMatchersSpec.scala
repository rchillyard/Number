...
//      println("===============================\n")
  ...
  it should "cancel addition and subtraction of e" in {
    ...
    val z = y + UniFunction(ConstE, Negate)
    matchSimpler(z).get shouldBe One
  }
  ...
  it should "eliminate √3 and -√3 in Aggregate" in {
    val x = Aggregate.total(3, root3, UniFunction(root3, Negate), -1)
    val p: em.Matcher[Aggregate, Expression] = em.complementaryTermsEliminatorAggregate
    val result: em.MatchResult[Expression] = p(x)
...
//    }
  ...
  it should "simplify aggregate 4a" in {
    val target: Expression = Aggregate(Sum, Seq(One, ConstE, UniFunction(ConstE, Negate)))
  ...
  it should "work for UniFunction" in {
    val target: CompositeExpression = UniFunction(Two * Two, Negate)
    //val result: Expression = em.simplifyTerms(target)
    val result: Expression = Expression.simplifyComponents(target).getOrElse(target)
    result shouldBe UniFunction(4, Negate)
  }
  ...
  it should "work for 7 + 2 - 3" in {
    ...
    result shouldBe em.Match(Aggregate.total(7, Two, UniFunction(Expression(3), Negate)))
  }
  it should "work for 7 * 2 * -3" in {
    val x: Expression = Expression(7) * 2 * -3
    //val p = em.matchBiFunction & em.biFunctionAggregator
  ...
  it should "cancel value and reciprocal 1" in {
    ...
    val x = Literal(Number.pi)
    val y = UniFunction(x, Reciprocal)
    val result = p(Product ~ x ~ y)
  ...
  it should "work for plus and minus" in {
    em.matchComplementaryExpressions(Sum ~ One ~ UniFunction(One, Negate)) shouldBe em.Match(Zero)
    em.matchComplementaryExpressions(Sum ~ UniFunction(One, Negate) ~ One) shouldBe em.Match(Zero)
    em.matchComplementaryExpressions(Sum ~ BiFunction(Two, One, Sum) ~ UniFunction(One, Negate)) shouldBe em.Match(Two)
    em.matchComplementaryExpressions(Sum ~ UniFunction(One, Negate) ~ BiFunction(Two, One, Sum)) shouldBe em.Match(Two)
    em.matchComplementaryExpressions(Sum ~ BiFunction(One, Two, Sum) ~ UniFunction(One, Negate)) shouldBe em.Match(Two)
    em.matchComplementaryExpressions(Sum ~ UniFunction(One, Negate) ~ BiFunction(One, Two, Sum)) shouldBe em.Match(Two)
  }
  it should "work for reciprocals" in {
    em.matchComplementaryExpressions(Product ~ Two ~ UniFunction(Two, Reciprocal)) shouldBe em.Match(One)
    em.matchComplementaryExpressions(Product ~ UniFunction(Two, Reciprocal) ~ Two) shouldBe em.Match(One)
    em.matchComplementaryExpressions(Product ~ BiFunction(Two, One, Product) ~ UniFunction(Two, Reciprocal)) shouldBe em.Match(One)
    em.matchComplementaryExpressions(Product ~ UniFunction(Two, Reciprocal) ~ BiFunction(Two, One, Product)) shouldBe em.Match(One)
    em.matchComplementaryExpressions(Product ~ BiFunction(One, Two, Product) ~ UniFunction(Two, Reciprocal)) shouldBe em.Match(One)
    em.matchComplementaryExpressions(Product ~ UniFunction(Two, Reciprocal) ~ BiFunction(One, Two, Product)) shouldBe em.Match(One)
  }
  ...
  it should "match 2" in {
    ...
    p(Sum ~ UniFunction(Zero, Cosine) ~ UniFunction(ConstPi, Cosine)) shouldBe em.Match(Zero)
  }
  ...
  it should "simplify √3 * -1 as -√3" in {
    ...
    expression.simplify shouldBe UniFunction(Expression(root3), Negate)
  }
  it should "simplify √3 * 1 as √3" in {
